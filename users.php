...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === "add" || $action === "edit") {
        ...
        if ($username === "" || $email === "" || !in_array($role, $roles, true)) {
            $error = "Role, username and email are required.";
...
if (isset($_GET["edit"])) {
    $eid = (int) $_GET["edit"];
    $stmt = $conn->prepare("SELECT user_id, username, role, email FROM users WHERE user_id = ?");
...
$rows = $conn->query("SELECT user_id, username, role, email FROM users ORDER BY user_id")->fetch_all(MYSQLI_ASSOC);
